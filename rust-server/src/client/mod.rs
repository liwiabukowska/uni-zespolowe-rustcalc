    ...
    C: Has<XSpanIdString>  + Clone + Send + Sync + 'static,
{
    ...
    async fn get_calc(
        &self,
        param_calculation: models::Calculation,
        context: &C) -> Result<GetCalcResponse, ApiError>
    {
        ...
        // Body parameter
        let body = param_calculation.as_xml();
        ...
        match response.status().as_u16() {
            200 => {
                ...
                let body = serde_json::from_str::<models::CalculationResult>(body).map_err(|e| {
                    ApiError(format!("Response body did not match the schema: {}", e))
                })?;
                Ok(GetCalcResponse::SuccessfulOperation
                    (body)
                )
            }
            400 => {
